...
### 19. v0：纯手写启发式基线 + 上榜 · 2026-06-17
新增 **v0**——完全不带神经网络、不做搜索的手写启发式（`heuristic_v0.py`）：只有「立即成五 / 立即挡五」反射 + 一步威胁/棋形评分（活四、冲四、活三、双威胁叉、中心倾向）。同一套打分在 `docs/engine.worker.js` 里逐字镜像，网页对手与被评测引擎落子完全一致（`399` 随机局面 Python↔JS 100% 对齐）。网页模型选择器加入 v0（无权重、秒载，深算滑杆置灰）。
🎯 **模型产出 · v0（手写启发式）**：对各模型各 `30` 局 @`128 sims`、颜色互换、随机开局——对 **v1 `30-0`（`1.000`）**、对 **v3 `0.683`**、对 **v4 `0.633`**、对 **v5 `0.283`**。固定 v1–v5 既有 Elo、对 v0 做单维最大似然拟合得 **Elo `1501`**，排名 **第 2**（仅次于 v5），头对头反超 v4/v3。一个下午能写完的规则基线，在 `128 sims` 下凭无瑕的局部战术吊打中段网络，只有 SOTA 的 v5 能稳定压过它——这正是「学习模型到底强在哪」的最好参照。评测脚本 `scripts/rate_v0.py` 可复现。

### 20. v6：纯 AlphaZero 更强一档 · 2026-06-18
**纯 AlphaZero 路线**（零先验、不注入任何人工规则、不增大网络）继续压榨 v5。诊断：v5 卡在 `policy_top1~0.757` 是**目标质量**受限（96 sims 的 Gumbel 目标），不是容量——更大的 v3/v4 反而停在更低。修法只用纯自对弈：warm-start v5、**自对弈搜索加码**（sims `96→160`、fast `32→48`、Gumbel considered `16→24`、full_search_prob `0.4→0.6`）→ 目标本身更干净、不再漏战术 → 网络学得更锐。仍是 `64×5`，**网页体积/速度不变**（fp16 仍 `1.17MB`）。先 50 轮探针（v6@140 对 v5 `0.817`），未停滞，再续 100 轮（`v6cont`，champion 链式叠 4 次晋升到 iter `175`）。**注意**：`considered` 调大让目标变软，`top1` 机械性掉到 `~0.65`（假象，别据此判断），真实信号看实战与 value（value_mae `0.42→0.30`）。
🎯 **模型产出 · v6（新 SOTA，网页默认）**：各 `40` 局 @`128 sims`、颜色互换——对 **v5 `0.725`（29-11）**、对 **v4 `0.900`**、对 **v3 `0.800`**、对 **v1 `1.000`**、对 **v0 `0.838`**（比 v5 的 0.717 更彻底压制手写启发式）。固定 v0–v5 既有 Elo 拟合得 **Elo `1751`，登顶第 1**（v6 `1751` > v5 `1607` > v0 `1501` > v4 `1426` > v3 `1358` > v1 `1000`）。同款轻量网、更狠的自对弈搜索 → 纯学习再上一档。训练 preset `v6-tiny-3080`/`v6cont-3080`、评测脚本 `scripts/rate_v6.py` 可复现。
